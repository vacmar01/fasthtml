...
pointing at the database’s filepath or endpoint.
...
If no `pk` is provided, `id` is assumed to be the primary key. We can
also construct a database table using a Dataclass.
...
We can also query on compound Primary keys, either individually or in
combination.
...
Attempting to delete a record with a compound primary key without all of
those keys throws a `NotFoundError` with a string that provides
information as to why the deletion failed.
...
> **Note**
>
> This behavior is automatic so long as a `__getitem__` method is
...
One more thing. Once `.xtra` is set the index field value becomes
immutable, in other words it can no longer be altered. Instead, while
the rest of an update will progress, the index field value will remain
the same.

To demonstrate let’s create a new record, a more realistic todo and
assign it to Charlie then attempt to update the `name` field to Braden.
...
Now we try to assign the record to Braden while also adding detail to
the record. The detail field now contains a value, but the `name` of the
record will remain as `Charlie`.
...
We are excited to see the MiniDataAPI specification implemented for many
different databases.
